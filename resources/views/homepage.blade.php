        ...
        @foreach($products as $product)
            <div class="card border-1 border-dark" style="width: 18rem;">
                <div class="d-block text-center h-75 d-inline-block">
                    <img src="../assets/photos/{{$product -> photo}}" class="card-img-top mw-75" alt="...">
                </div>
                <div class="card-body">
                    <span class="d-flex"><p class="card-title w-50 font-weight-bold">Nom:</p><p class="m-0 px-2 w-50 text-right font-weight-bold">{{$product -> name}}</p></span>
                    <span class="d-flex"><p class="card-title w-50 font-weight-bold">Extension:</p><p class="m-0 px-2 w-50 text-right font-weight-bold">{{$product -> extension}}</p></span>
                    ...
                    <boutton onclick="window.location='{{ url("/product/{$product->id}") }}'" class="btn btn-block w-100">Détail</boutton>
                </div>
            </div>
        @endforeach
